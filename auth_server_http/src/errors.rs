...
use http::{header::InvalidHeaderName, StatusCode, header::InvalidHeaderValue};
...
impl IntoResponse for ServiceError {
    fn into_response(self) -> Response {
        match self {
            Self::Unauthorized => (StatusCode::UNAUTHORIZED, "Not Authorized").into_response(),
            Self::BadRequest(s) => (StatusCode::BAD_REQUEST, s).into_response(),
            e => (StatusCode::INTERNAL_SERVER_ERROR, format_sstr!("{e}")).into_response()
        }
